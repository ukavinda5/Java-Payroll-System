...
public class Leave {
    ...
    public boolean setleave() {
        try {
            String sqlin = "INSERT INTO tbl_leave VALUES('" + id + "','" + annual + "','" + casual + "','" + optional + "')";
            System.out.println(sqlin);
            int inserted = objSQLRun.sqlUpdate(sqlin);
            if (inserted > 0) {
                JOptionPane.showMessageDialog(null, "Your Leaves has been Recorded", "", 1);
                return true;
            } else {
                JOptionPane.showMessageDialog(null, "Error in Applying Leave", "ERROR", 0);
                return false;
